...
AddiVortes <- function(y, x, m = 200,
  ...
  #### Ensuring sigma is vectorised, and dealing with sigma NULL ---------------
  if (is.null(sd)) {
    if (scaleX) sd <- 0.8
    else {
  ...
  tess <- sapply(dim, function(i) {
    list(matrix(rnorm(1, mus[i], sd[i])))
  })
